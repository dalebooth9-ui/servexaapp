...
export async function generateJobSheetPdf(
  ...
  for (const galField of galleryFields) {
    ...
    const entries: Entry[] = rows.map((row: any, idx: number) => {
      ...
      // Free-text breakdown column (e.g. handwritten "HALLWAY x2, KITCHEN/LIVING x2 …")
      const breakdownFree = String(row?.[breakdownCol?.id] ?? row?.room_breakdown ?? row?.breakdown ?? "").trim();
      // Prefer per-room-column derived list; fall back to free-text breakdown.
      const breakdown = breakdownParts.length > 0 ? breakdownParts.join(", ") : breakdownFree;
      // Notes = notes column + any descriptive overflow from the status column
      // (e.g. "Refused access but could see sprinklers inside" — keep the full
      // sentence, don't lose it just because we classified the badge as "refused").
      ...
      // If breakdown column was actually a free-text note (e.g. "NO SPRINKLER
      // HEADS") and no heads/rooms parsed, keep it visible even for non-gained rows.
