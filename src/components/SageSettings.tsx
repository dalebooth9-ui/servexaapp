...
export default function SageSettings() {
  ...
  const checkStatus = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(
        `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/sage-auth?action=status`,
        {
          headers: {
  ...
  useEffect(() => {
    ...
    const connected = searchParams.get("sage_connected");
    const error = searchParams.get("sage_error");
    const business = searchParams.get("business");

    if (connected === "true") {
      toast.success(`Connected to Sage${business ? ` (${business})` : ""}`);
      searchParams.delete("sage_connected");
      searchParams.delete("business");
      setSearchParams(searchParams, { replace: true });
      checkStatus();
    } else if (error) {
      toast.error(`Sage connection failed: ${error}`);
      searchParams.delete("sage_error");
      setSearchParams(searchParams, { replace: true });
    }
  ...
  const handleConnect = async () => {
    ...
    try {
      const session = await supabase.auth.getSession();
      const res = await fetch(
        `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/sage-auth?action=authorize`,
        {
          headers: {
            Authorization: `Bearer ${session.data.session?.access_token}`,
            apikey: import.meta.env.VITE_SUPABASE_PUBLISHABLE_KEY,
      ...
      else toast.error(result.error || "Failed to start Sage sign-in");
    } catch (err: any) {
  ...
  const handleDisconnect = async () => {
    ...
    try {
      const session = await supabase.auth.getSession();
      const res = await fetch(
        `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/sage-auth?action=disconnect`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${session.data.session?.access_token}`,
      ...
      const result = await res.json();
      if (result.success) {
        toast.success("Disconnected from Sage");
        await checkStatus();
      } else {
  ...
  const runSync = async (action: SyncAction) => {
    ...
    try {
      ...
      if (action === "import_contacts") {
        ...
        toast.success(`Pulled from Sage: ${data.created} new, ${data.updated} updated`);
      } else if (action === "sync_payments") {
        toast.success(`Updated ${data.updated} invoice(s) (${data.checked} checked)`);
      } else {
        const bits = [`${data.pulled} added`, `${data.pushed} sent to Sage`];
    ...
    <Card>
      <CardHeader>
        <div className="flex items-center gap-2">
          ...
          <CardTitle className="text-lg">Sage Business Cloud Accounting</CardTitle>
          {status?.connected && (
        ...
        <CardDescription>
          Sync invoices, quotes, customers, payments and products with Sage Accounting.
        </CardDescription>
      </CardHeader>
          ...
          <>
            <div className="rounded-lg border p-3 text-sm">
              <p className="font-medium">
                Connected to: {status.business_name || "Sage business"}
              </p>
            ...
            <p className="text-xs text-muted-foreground">
              Send an individual invoice or quote from its own page using “Send to Sage”.
            </p>
