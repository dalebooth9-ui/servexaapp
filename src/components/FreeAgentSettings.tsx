...
type SyncAction = "import_contacts" | "pull_invoices" | "sync_payments";
...
export default function FreeAgentSettings() {
  ...
  const [status, setStatus] = useState<FreeAgentStatus | null>(null);
  ...
  const checkStatus = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(
        `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/freeagent-auth?action=status`,
        {
          headers: {
  ...
  useEffect(() => {
    ...
    const connected = searchParams.get("freeagent_connected");
    const error = searchParams.get("freeagent_error");
    const company = searchParams.get("company");

    if (connected === "true") {
      toast.success(`Connected to FreeAgent${company ? ` (${company})` : ""}`);
      searchParams.delete("freeagent_connected");
      searchParams.delete("company");
      setSearchParams(searchParams, { replace: true });
      checkStatus();
    } else if (error) {
      toast.error(`FreeAgent connection failed: ${error}`);
      searchParams.delete("freeagent_error");
      setSearchParams(searchParams, { replace: true });
    }
  ...
  const handleConnect = async () => {
    ...
    try {
      const session = await supabase.auth.getSession();
      const res = await fetch(
        `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/freeagent-auth?action=authorize`,
        {
          headers: {
            Authorization: `Bearer ${session.data.session?.access_token}`,
            apikey: import.meta.env.VITE_SUPABASE_PUBLISHABLE_KEY,
      ...
      else toast.error(result.error || "Failed to start FreeAgent sign-in");
    } catch (err: any) {
  ...
  const handleDisconnect = async () => {
    ...
    try {
      const session = await supabase.auth.getSession();
      const res = await fetch(
        `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/freeagent-auth?action=disconnect`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${session.data.session?.access_token}`,
  ...
  const runSync = async (action: SyncAction) => {
    ...
    try {
      ...
      if (action === "import_contacts") {
        ...
        toast.success(`Pulled from FreeAgent: ${data.created} new, ${data.updated} updated`);
      } else {
    ...
    <Card>
      <CardHeader>
        <div className="flex items-center gap-2">
          ...
          <CardTitle className="text-lg">FreeAgent</CardTitle>
          {status?.connected && (
        ...
        <CardDescription>
          Sync invoices, quotes, customers and payments with FreeAgent.
        </CardDescription>
      </CardHeader>
          ...
          <>
            <div className="rounded-lg border p-3 text-sm">
              <p className="font-medium">
                Connected to: {status.company_name || "FreeAgent company"}
              </p>
            ...
            <p className="text-xs text-muted-foreground">
              Send an individual invoice or quote from its own page using “Send to FreeAgent”.
              FreeAgent has no product list, so there is no product sync.
            </p>
