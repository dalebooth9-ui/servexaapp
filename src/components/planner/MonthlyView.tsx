                      ...
                      <div>
                        <div className="mb-1 flex items-center gap-1.5">
                          ...
                          <span className="text-[10px] text-muted-foreground">
                            {groupedUnallocated.reduce((sum, [, arr]) => sum + (arr as Job[]).length, 0)}
                          </span>
                        </div>
                        <div className="space-y-1">
                          {groupedUnallocated.flatMap(([, areaJobs]) => areaJobs as Job[]).map((job) => (
                            <DraggableJobCard key={`unalloc-${job.id}`} job={job} />
                          ))}
                        </div>
                      </div>
                    )}
