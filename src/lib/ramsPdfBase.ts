...
/**
 * checkPageBreak — if y + neededMm would exceed SAFE_BOTTOM,
 * writes footer, starts a new page with header, increments pageRef.num.
 */
...
export async function buildSharedMethodSections(
  ...
  // Always start method statement on a fresh page with header
  newPage(doc);
  ...
  for (let i = 0; i < sections.taskSpecificOps.length; i++) {
    const lines = doc.splitTextToSize(sections.taskSpecificOps[i], CONTENT_W - 8);
    const itemH = lines.length * (8.5 * 0.352778 + 1.2) + 1;
    y = await checkPageBreak(doc, y, itemH + 1, logoImg, currentPageRef, totalPages);
    doc.text(`${i + 1}.`, ML + 2, y);
  ...
  y = await checkPageBreak(doc, y, 14, logoImg, currentPageRef, totalPages);
  ...
  y = para(doc, sections.personnel, ML, y, CONTENT_W); y += 2;

  y = await checkPageBreak(doc, y, 12, logoImg, currentPageRef, totalPages);
  doc.setFont("helvetica", "bold"); doc.setFontSize(9); doc.text("3.2 Supervision", ML, y); y += 4;
  y = para(doc, "NAME AND CONTACT: Mr Martin Whatmough (SSSTS), Tel: 07989436509", ML, y, CONTENT_W); y += 2;

  y = await checkPageBreak(doc, y, 12 + sections.plantAndEquipment.length * 5, logoImg, currentPageRef, totalPages);
  ...
  y = await checkPageBreak(doc, y, 12 + sections.significantRisks.length * 4, logoImg, currentPageRef, totalPages);
