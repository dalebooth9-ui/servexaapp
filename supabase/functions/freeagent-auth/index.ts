...
import { FREEAGENT_AUTH_URL, serviceClient } from "../_shared/freeagent.ts";
  ...
  try {
    ...
    if (action === "status") {
      const { data } = await svc
        ...
        .select("id, company_name, company_url, token_expires_at, connection_error, created_at")
        .eq("org_id", orgId)
      ...
      return json({
        ...
        company_url: (data as any)?.company_url ?? null,
        connected_at: (data as any)?.created_at ?? null,
        connection_error: (data as any)?.connection_error ?? null,
        token_expired: data ? new Date((data as any).token_expires_at) < new Date() : false,
      });
    }

    if (!isAdmin) {
      return json({ error: "Only organisation admins can manage the FreeAgent connection" }, 403);
    }
    ...
    if (action === "authorize") {
      ...
      // FreeAgent grants full access on approval — no scope parameter.
      const authUrl = `${FREEAGENT_AUTH_URL}?client_id=${encodeURIComponent(clientId)}` +
        `&response_type=code` +
    ...
    if (action === "disconnect") {
      await svc.from("freeagent_connections").delete().eq("org_id", orgId);
    ...
    console.error("FreeAgent auth error:", err);
