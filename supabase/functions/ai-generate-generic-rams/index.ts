...
serve(async (req) => {
  ...
  try {
    const { contract_name, site_name, client, description, factors } = await req.json();
...
Produce a draft Risk Assessment + Method Statement for the works described, tailored to fire protection trades.
...
Risk ratings use 1-5 likelihood (L) and 1-5 severity (S); risk = L*S. Provide pre-control and post-control ratings.`;
    ...
    const userPrompt = `Job: ${contract_name || "(no name)"}
Site: ${site_name || "(no site)"}
...
Generate hazards covering the described works AND every flagged factor.`;
          ...
          function: {
            ...
            parameters: {
              ...
              properties: {
                ...
                sequence_of_works: { type: "array", items: { type: "string" } },
                ppe: { type: "array", items: { type: "string" } },
                plant_equipment: { type: "array", items: { type: "string" } },
                emergency_arrangements: { type: "string" },
              },
              required: ["risk_rows", "sequence_of_works", "ppe", "plant_equipment", "emergency_arrangements"],
              additionalProperties: false,
            },
          },
        }],
    ...
    console.error("ai-generate-generic-rams error", e);
