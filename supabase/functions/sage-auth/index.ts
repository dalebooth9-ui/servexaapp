...
import { SAGE_API_FILTER, SAGE_AUTH_URL, SAGE_SCOPE, serviceClient } from "../_shared/sage.ts";
  ...
  try {
    ...
    if (action === "status") {
      const { data } = await svc
        ...
        .select("id, business_name, business_id, token_expires_at, connection_error, created_at")
        .eq("org_id", orgId)
      ...
      return json({
        ...
        business_id: (data as any)?.business_id ?? null,
    ...
    if (!isAdmin) {
      return json({ error: "Only organisation admins can manage the Sage connection" }, 403);
    }
    ...
    if (action === "authorize") {
      ...
      if (!clientId || !clientSecret) {
        return json({ error: "Sage credentials are not configured" }, 500);
      }
      ...
      const authUrl = `${SAGE_AUTH_URL}?filter=${encodeURIComponent(SAGE_API_FILTER)}` +
        `&client_id=${encodeURIComponent(clientId)}` +
        `&response_type=code&scope=${encodeURIComponent(SAGE_SCOPE)}` +
    ...
    if (action === "disconnect") {
      await svc.from("sage_connections").delete().eq("org_id", orgId);
    ...
    console.error("Sage auth error:", err);
