...
function extractStoragePath(url: string, bucket: string): string | null {
  try {
    const u = new URL(url);
    // matches /object/sign/<bucket>/<path> or /object/public/<bucket>/<path>
...
async function fetchExcelBytes(
  ...
  // Try to extract storage path and use service-role download (bypasses RLS)
  ...
  // Fallback: plain fetch (works for signed URLs / public URLs)
  console.log("Downloading via fetch:", fileUrl.slice(0, 80));
  const res = await fetch(fileUrl, { signal: AbortSignal.timeout(20000) });
  if (!res.ok) throw new Error(`Failed to fetch file: ${res.status} ${res.statusText}`);
  return await res.arrayBuffer();
}

async function fetchExcelText(excelUrl: string, bucket: string, supabase: ReturnType<typeof createClient>): Promise<string> {
...
serve(async (req) => {
  ...
  try {
    ...
    console.log("parse-costing-sheet called, job_id:", job_id, "file_url:", file_url.slice(0, 80));
    ...
    if (job_id) {
      ...
      if (allocated_days != null) {
        const { error: daysError } = await supabase.from("jobs").update({ allocated_days } as any).eq("id", job_id);
