CREATE OR REPLACE FUNCTION public.confirm_paper_scan_job(_template_id uuid, _customer_id uuid, _site_id uuid, _completed_at timestamp with time zone, _date_known boolean, _category text, _responses jsonb, _customer_po text DEFAULT NULL::text, _existing_job_id uuid DEFAULT NULL::uuid, _batch_item_id uuid DEFAULT NULL::uuid, _override_name text DEFAULT NULL::text)
...
BEGIN
  ...
  IF _existing_job_id IS NOT NULL THEN
    UPDATE public.jobs j
     ...
     WHERE j.id = _existing_job_id AND j.org_id = v_org
